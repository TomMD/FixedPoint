...
instance (Arbitrary a) => Arbitrary (GenericFixedPoint a b) where
	arbitrary = fmap FixedPoint arbitrary

instance Arbitrary Word128 where
	arbitrary = W128 <$> arbitrary <*> arbitrary

instance Arbitrary a => Arbitrary (BigWord a) where
...
aeq a b = logBase 2 a < 63 ==> let e = 0.001 in if abs (a - b) < e then True else trace (show (a,b,abs (a-b), e)) False
...
testFP = [T "sub" propSub, T "add" propAdd, T "div" propDiv, T "mul" propMul
	,T "conv" propConv, T "erf" propErf, T "exp" propExp]
...
divW256,remW256 :: Word256 -> Word256 -> Property
...
divW128,remW128 :: Word128 -> Word128 -> Property
